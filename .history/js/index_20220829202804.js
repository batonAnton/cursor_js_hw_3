...
function convertCurrency (typeValue){
    let dollars = 39.9;
    let uahResult = +typeValue.slice(0, typeValue.indexOf("UAH")) / dollars + "$";
    let dollarsResult = +typeValue.slice(0, typeValue.indexOf("$")) * dollars + "UAH";
    let res;
    
       
        if (typeValue.includes("UAH")) {
            res =   uahResult.toFixed(2);
          } else if (typeValue.includes("$")) {
            res = dollarsResult.toFixed(2);
          } return res;

...
function convertCurrency (typeValue){
    ...
    let res;
        ...
        if (typeValue.includes("UAH")) {
            res =   +typeValue.slice(0, typeValue.indexOf("UAH")) / dollars + "$";
          } else if (typeValue.includes("$")) {
            res = +typeValue.slice(0, typeValue.indexOf("$")) * dollars + "UAH";
          } return res;
...
let typeValue="100 UAH";

document.writeln(`Функція №7: Гривні в доларах будуть - ${convertCurrency(typeValue)} uah.  Долари в гривнях будуть`);
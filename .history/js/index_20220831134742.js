...
function deleteLetters(removeLetter, exampleWord) {
  ...
  for (let i = 0; i < exampleWord.length; i++) {
    exampleWord[i] == removeLetter ;
    result : (result += exampleWord[i]);
  }
  return result;
}

const removeLetter = "t";
const exampleWord = "anatoliy";
document.writeln(`<br> Функція №9: У слові ${exampleWord} видалити літеру ${removeletter} - ${deleteLetters()}....`);
